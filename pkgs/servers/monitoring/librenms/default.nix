...
in package.override rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = pname;
    rev = "${version}";
    sha256 = "sha256-AjnTQSsZ7FAOpLdIA+RCp41KuFqBOr48gp1JTXkPncY=";
  };
  ...
  postInstall = ''
    ...
    substituteInPlace $out/LibreNMS/wrapper.py --replace '/usr/bin/env php' '${php82}/bin/php'
    substituteInPlace $out/LibreNMS/__init__.py --replace '"/usr/bin/env", "php"' '"${php82}/bin/php"'
    substituteInPlace $out/snmp-scan.py --replace '"/usr/bin/env", "php"' '"${php82}/bin/php"'

    wrapProgram $out/daily.sh --prefix PATH : ${php82}/bin
